...
class User:
  ...
  def get_by_id(cls,data):
    query = """
      SELECT * FROM users JOIN parties ON users.id = parties.user_id WHERE users.id = %(id)s;
    """
    results = connectToMySQL(DATABASE).query_db(query,data)
    if results:
      ...
      for row in results:
        ...
        this_party_instance = party_model.Party(party_data)
        party_list.append(this_party_instance)
      this_user.parties = party_list
      return this_user
    return False
