...
def users_log():
  ...
  if not bcrypt.check_password_hash(user_in_db.password, request.form["password"]):
    flash("Invalid credentials", "log")
    return redirect("/")
  session["user_id"] = user_in_db.id #stores user in session
  return redirect("/dashboard")
...
def dash():
  ...
  this_show = Show.get_by_id({"id":id}) #stores id in a dict/list
  show_preference = User.get_shows_by_user_id(data)
  return render_template("welcome.html", logged_user=logged_user, all_shows=all_shows, this_show=this_show, show_preference=show_preference)